...
class _InventoryPanelState extends State<_InventoryPanel>
  ...
  Future<void> _confirmUse(InventoryItem item) async {
          ...
          child: _OverlayPanel(
            child: SizedBox(
              ...
              child: Column(
                ...
                children: [
                  Text(
                    strings.tr(zh: '確認使用道具', en: 'Use Item'),
                    style: TextStyle(
                  ...
                  Text(
                    strings.tr(
                      zh: '確定要使用「${item.name}」嗎？',
                      en: 'Use "${item.name}" now?',
                    ),
    ...
    try {
      ...
      setState(() {
        _successText = AppStrings.of(context).tr(
          zh: '兌換成功！${result.itemName} 剩餘 ${result.remainingQuantity}',
          en: 'Used ${result.itemName}. ${result.remainingQuantity} left.',
        );
      ...
      setState(() {
        _errorText = AppStrings.of(
          context,
        ).tr(zh: '使用失敗：$error', en: 'Use failed: $error');
      });
    } finally {
      if (mounted) {
        setState(() {
  ...
  Widget _buildInventoryActionRow({
    ...
    return Row(children: [quantityChip, const Spacer(), useButton]);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        Text(
          strings.tr(zh: '收藏背包', en: 'Collection Bag'),
          style: TextStyle(
            color: AppColors.inkBrown,
            fontSize: 22,
            fontWeight: FontWeight.w900,
        ...
        Text(
          strings.tr(
            zh: '點擊道具即可使用。使用後會同步記錄到家庭動態。',
            en: 'Tap an item to use it. The result is also logged to the family feed.',
          ),
          style: TextStyle(
            ...
            data: (items) {
              if (items.isEmpty) {
                return Center(
                  child: Text(
                    strings.tr(
                      zh: '收藏背包目前是空的，先去獎勵站兌換一些道具吧。',
                      en: 'Your collection bag is empty. Visit Rewards to claim something first.',
                    ),
                    style: TextStyle(
                      color: AppColors.inkBrown,
